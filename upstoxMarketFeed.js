    ...
    */
    ...
    async loadProtobufSchema() {
        try {
            const schemaContent = this.getOfficialProtobufSchema();
            this.protoRoot = await protobuf.parse(schemaContent).root;
            this.logDebug('Protobuf schema loaded from embedded definition');
        } catch (error) {
            console.error('❌ Failed to parse protobuf schema:', error.message);
            throw error;
        }
        
        this.FeedResponse = this.protoRoot.lookupType(
            'com.upstox.marketdatafeederv3udapi.rpc.proto.FeedResponse'
        );
        this.logDebug('FeedResponse type resolved successfully');
    }
